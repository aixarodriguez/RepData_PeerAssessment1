...
Firt the data was download from url <a href="https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip" target="_blank" rel="noopener nofollow">Activity monitoring data</a> and read csv data from zip file. Two dataset are defined, first with the original data and the second without missing values, aditionaly the date column was transform into a format suitable for analysis.
...
First calculate the number of steps by day.
...
ggplot(steps_by_5min,aes(x=Group.1,y=x))+geom_line(color="#800060",linetype=1,size=1)+ggtitle("Average steps in 5-minute interval")+ylab("Mean Steps")+xlab("5-minuts Interval")
```
